...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        # print(node.op, " --(sub)--> " ,end="")
        # for ip in node.inputs:
        #     print(ip.op, ", ", end="")
        # print()
        # setattr(node,'grad',None)
        grad_list = node_to_output_grads_list[node]
        grads = sum(grad_list)
        node.grad = grads
        if(node.op is not None and len(node.inputs) > 0):
            input_grads = node.op.gradient_as_tuple(grads, node)
            # pdb.set_trace()
            for ipNode, ipGrad in zip(node.inputs, input_grads):
                if(ipNode not in node_to_output_grads_list.keys()):
                    node_to_output_grads_list[ipNode] = [ipGrad]
                else:
                    node_to_output_grads_list[ipNode].append(ipGrad)
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    for node in node_list:
        visited = {}
        sub_top_list = []
        topo_sort_dfs(node, visited, sub_top_list)

    return_list.extend(sub_top_list)
    return return_list
    ### END YOUR SOLUTION
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    for ch in node.inputs:
        if(ch in visited.keys()):
            pass
        else:
            topo_sort_dfs(ch, visited, topo_order)
